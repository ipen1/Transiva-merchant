...
public class MerchantOperationsActivity extends MerchantBaseActivity {
    ...
    private void load(){new Thread(()->{try{JSONObject r=new JSONObject(get(BASE+"merchant_operations.php?v="+System.currentTimeMillis()));runOnUiThread(()->apply(r.optJSONObject("operations")));}catch(Exception e){runOnUiThread(()->pauseState.setText("Gagal memuat pengaturan operasional."));}}).start();}
    ...
    private void pause(int mins){send("pause",mins,null);} private void resume(){send("resume",0,null);} private void send(String action,int mins,JSONArray schedule){new Thread(()->{try{JSONObject p=new JSONObject();p.put("action",action);if(mins>0)p.put("minutes",mins);if(schedule!=null)p.put("schedule",schedule);JSONObject r=new JSONObject(postJson(BASE+"merchant_operations.php",p));runOnUiThread(()->{toast(r.optString("message","Tersimpan"));load();});}catch(Exception e){runOnUiThread(()->alert("Error","Gagal menyimpan pengaturan."));}}).start();}
