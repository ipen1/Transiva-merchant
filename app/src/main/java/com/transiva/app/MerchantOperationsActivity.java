...
public class MerchantOperationsActivity extends MerchantBaseActivity {
    ...
    private void load(){MerchantNetworkExecutor.execute(()->{try{JSONObject r=new JSONObject(get(BASE+"merchant_operations.php?v="+System.currentTimeMillis()));runOnUiThread(()->apply(r.optJSONObject("operations")));}catch(Exception e){runOnUiThread(()->pauseState.setText("Gagal memuat pengaturan operasional."));}});}
