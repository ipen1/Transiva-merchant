...
public class MerchantAnalyticsActivity extends MerchantBaseActivity {
    ...
    private void load(){dailyList.removeAllViews();topList.removeAllViews();dailyList.addView(card("Memuat analitik..."));new Thread(()->{try{JSONObject res=new JSONObject(get(BASE+"merchant_analytics.php?v="+System.currentTimeMillis()));runOnUiThread(()->show(res));}catch(Exception e){runOnUiThread(()->{dailyList.removeAllViews();dailyList.addView(card("Koneksi gagal."));});}}).start();}
