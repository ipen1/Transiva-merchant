...
final class MerchantLoginRepository {
    ...
    private static void uploadFcm(Activity a,int userId,String username,String role,String token){
        if(token==null||token.trim().isEmpty())return;
        String key="fcm-sync-"+userId+"-"+token.trim().hashCode();
        boolean accepted=MerchantNetworkExecutor.executeWrite(key,()->{
            HttpURLConnection c=null;
            try{
                c=(HttpURLConnection)new URL(SAVE_FCM_URL).openConnection();c.setRequestMethod("POST");c.setConnectTimeout(TIMEOUT_MS);c.setReadTimeout(TIMEOUT_MS);c.setDoInput(true);c.setDoOutput(true);c.setUseCaches(false);c.setInstanceFollowRedirects(false);c.setRequestProperty("Content-Type","application/json; charset=UTF-8");c.setRequestProperty("Accept","application/json");MerchantApiClient.applySecurity(a,c);
                JSONObject p=new JSONObject();p.put("user_id",userId);p.put("id",userId);p.put("username",username);p.put("role",role);p.put("fcm_token",token.trim());p.put("platform","android_native");p.put("installation_uuid",DeviceIdentityManager.getInstallationUuid(a));p.put("manufacturer",Build.MANUFACTURER);p.put("model",Build.MODEL);p.put("android_version",Build.VERSION.RELEASE);try{p.put("app_version",a.getPackageManager().getPackageInfo(a.getPackageName(),0).versionName);}catch(Exception ignored){p.put("app_version","unknown");}
                try(BufferedWriter w=new BufferedWriter(new OutputStreamWriter(c.getOutputStream(),StandardCharsets.UTF_8))){w.write(p.toString());}
                int code=c.getResponseCode();Log.d(TAG,"FCM upload HTTP="+code+", body="+read(code>=200&&code<300?c.getInputStream():c.getErrorStream()));
            }catch(Exception e){Log.e(TAG,"Upload FCM gagal",e);}finally{if(c!=null)c.disconnect();}
        });
        if(!accepted) Log.w(TAG,"FCM sync sedang berjalan atau antrean penuh.");
    }
