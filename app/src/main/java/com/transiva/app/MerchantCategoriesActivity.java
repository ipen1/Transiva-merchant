...
public class MerchantCategoriesActivity extends MerchantBaseActivity {
    ...
    private void load(){ list.removeAllViews(); list.addView(card("Memuat kategori...")); new Thread(()->{ try{ JSONObject r=new JSONObject(get(BASE+"merchant_categories.php?v="+System.currentTimeMillis())); runOnUiThread(()->show(r)); }catch(Exception e){runOnUiThread(()->{list.removeAllViews();list.addView(card("Gagal memuat kategori."));});}}).start(); }
    ...
    private void edit(JSONObject c){ final EditText input=new EditText(this); input.setInputType(InputType.TYPE_CLASS_TEXT); input.setHint("Contoh: Makanan Utama"); if(c!=null)input.setText(c.optString("name")); new AlertDialog.Builder(this).setTitle(c==null?"Tambah Kategori":"Edit Kategori").setView(input).setNegativeButton("Batal",null).setPositiveButton("Simpan",(d,w)->{String name=input.getText().toString().trim(); if(name.isEmpty())return; new Thread(()->{try{JSONObject p=new JSONObject();p.put("action","save");p.put("name",name);if(c!=null)p.put("id",c.optInt("id"));JSONObject r=new JSONObject(postJson(BASE+"merchant_categories.php",p));runOnUiThread(()->{toast(r.optString("message","Tersimpan"));load();});}catch(Exception ex){runOnUiThread(()->alert("Error","Gagal menyimpan kategori."));}}).start();}).show(); }
    private void remove(int id){ new AlertDialog.Builder(this).setTitle("Hapus kategori?").setMessage("Menu yang sudah memakai nama kategori tetap aman.").setNegativeButton("Batal",null).setPositiveButton("Hapus",(d,w)->new Thread(()->{try{JSONObject p=new JSONObject();p.put("action","delete");p.put("id",id);JSONObject r=new JSONObject(postJson(BASE+"merchant_categories.php",p));runOnUiThread(()->{toast(r.optString("message","Dihapus"));load();});}catch(Exception e){runOnUiThread(()->alert("Error","Gagal menghapus kategori."));}}).start()).show(); }
}
